...
def test_check_bad_examples_stats():
    ...
    for curr_img_path in BAD_ANGLE_IMAGES_LIST_ALL:
        ...
        if(not angle_is_good):
            correct += 1
    assert correct / counter_all >= 0.9, f"Expected 80% or more correct answers, got {correct / counter_all}" 
...
def test_check_good_examples_stats():
    ...
    assert correct / counter_all >= 0.85, f"Expected 80% or more correct answers, got {correct / counter_all}" 